...
| GAP-029 | OCR provider interface contract enforcement | OCR provider abstraction control is now documented, but explicit plan-carried OCR instruction validation, normalized provider adapter enforcement, and governed fallback-chain execution are not yet implemented end-to-end in runtime code. | Design constraint is now documented, but runtime enforcement remains incomplete. | Hidden provider defaults, unsafe fallback behavior, and provider-shaped payload drift. | High | Treat current OCR runtime as bootstrap-only and reject any interpretation that current code already satisfies the full provider-interface contract. | Implement explicit OCR instruction validation, provider adapter layer, normalized result contract enforcement, and governed fallback-chain execution in runtime code. | OPEN |
---
