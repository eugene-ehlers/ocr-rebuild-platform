...
| GAP-003 | Downstream execution | Real AWS ECS/Lambda invocation was previously unavailable and execution used only local service stubs. | Dual-mode execution active from Phase 20: local mode works and aws bridge mode exists, but live AWS runtime invocation is still not wired. | Infrastructure execution is still incomplete, but bridge path is now governed. | Medium | Use local mode for working execution and aws mode for controlled bridge testing. | Wire live ECS/Lambda invocation behind the AWS bridge. | PARTIAL |
