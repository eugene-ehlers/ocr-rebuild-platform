...
| GAP-003 | Downstream execution | Real AWS ECS/Lambda invocation was previously unavailable and execution used only local service stubs. | Dual-mode execution active from Phase 20. ECS repos, log groups, and task definitions now exist, but live runtime remains incomplete because new worker images have not yet been successfully packaged and executed end-to-end. | Infrastructure execution is partially governed but still not operationally complete. | Medium | Use local mode for working execution and aws mode for controlled bridge testing. Capture runtime packaging/import failures explicitly and do not retry failed import logic blindly. | Complete worker image runtime packaging, push images to ECR, then validate live ECS invocation. | PARTIAL |
